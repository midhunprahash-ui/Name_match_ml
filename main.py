...
SCORE_SIMILARITY_THRESHOLD = 1.0 
...
def compute_fuzzy_score(username, employee_name, first_name, last_name, emp_id):
    ...
    composite = (
        ...
        (avg_first_match * 0.2) +    
        (avg_last_match * 0.2) +     
        (soundex_match_last * 8) +   
        (metaphone_match_last * 8) + 
        (soundex_match_first * 8) +  
        (metaphone_match_first * 8) + 
        number_match_bonus           
    )
        ...
        for i, input_username in enumerate(input_usernames):
            ...
            if matches_to_add.empty:
                ...
                for rank_idx, (_, match_row) in enumerate(matches_to_add.iterrows()):
                    match_type = ''
                    if rank_idx == 0:
                        if match_row['fuzzy_score'] >= TOP_MATCH_THRESHOLD: 
                    ...
                    final_output_rows.append({
                        ...
                        'confidence_score': f"{match_row['fuzzy_score']:.2f}%", 
